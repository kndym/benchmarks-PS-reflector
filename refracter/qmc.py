...
The header files contain large static arrays of 3D points on the sphere,
generated by Quasi-Monte Carlo sampling.  This module reads them once and
caches the results so every call after the first is essentially free.

Public API
----------
load_main_cloud()   -> (x, y) each shape (16488, 3)
load_small_cloud()  -> (x_small, y_small) each shape (381, 3)
load_push_cloud()   -> shape (16488, 3)
"""
