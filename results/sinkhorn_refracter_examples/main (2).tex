...
In the code, the temperature parameter $\varepsilon$ is represented as $k = 1/\varepsilon$ (an integer), so small $\varepsilon$ corresponds to large $k$. The updates are computed entirely in log-domain using \texttt{logsumexp} for numerical stability. Equations \eqref{eq:g_update}--\eqref{eq:f_update} become
\begin{align}
    \mathrm{lse}_g[j] &= \mathrm{logsumexp}_i\!\bigl(k \cdot f[i] + \log p[i] - k \cdot C[i,j]\bigr), \\
    g_{\mathrm{new}}[j] &= g[j]/2 - \mathrm{lse}_g[j]/(2k),
\end{align}
and symmetrically for $f$. The factor of $1/2$ in the update (rather than a full step) arises from the C++ ``absorption'' scheme that folds the multiplicative kernel factor back into the additive potential after every half-step; the fixed point is identical to the standard Sinkhorn fixed point.
...
\begin{enumerate}
    \item \textbf{Convergence criterion:} $\max_i |f^{(n+1)}[i] - f^{(n)}[i]| < 10^{-5}$. In the code this quantity is \texttt{maxdif}, computed as $\max_i |\log F[i]|/k$ where $F[i]$ is the multiplicative absorption factor.
...
The azimuthal coordinate is sampled from its interval and the cosine of the
polar coordinate is sampled uniformly.  Sampling $\cos\phi$, rather than
$\phi$ itself, makes the point cloud uniform with respect to spherical area.
...
\[
    p_i=q_j=\frac{1}{N_K}.
\]
For a nonuniform density, the code evaluates the density at each point and
normalizes the resulting values so that both weight vectors have total mass
one.
...
\begin{align*}
    ...
    \texttt{cap\_thr} &= 10^{-5}.
\end{align*}
Here $k$ is the inverse regularization parameter, so the corresponding
temperature is $\varepsilon=1/k$.  The final main solve therefore uses
$\varepsilon_{\mathrm{final}}=1/k_{\mathrm{final}}$.

\subsection{Log-domain Sinkhorn iterations}
...
\]
Substituting $k=1/\varepsilon$, $\log p_i$ and $\log q_j$ gives the form used
by the two mirrored log-sum-exp routines:
\begin{align}
    \operatorname{lse}_g[j]
        &=\operatorname{logsumexp}_i\bigl(kf_i+\log p_i-kC_{ij}\bigr), \\
    \operatorname{lse}_f[i]
        &=\operatorname{logsumexp}_j\bigl(kg_j+\log q_j-kC_{ij}\bigr).
\end{align}
The logarithm of the sum is evaluated after subtracting the largest exponent
internally, which avoids overflow and underflow when $k$ is large.  The code
also computes the cost matrix in row blocks controlled by 
\texttt{chunk\_size}; it never stores the full $N_K\times N_K$ matrix.

In the repository implementation, \texttt{sinkhorn\_step} first forms the
target update from $\operatorname{lse}_g$, uses that updated target potential
for the source update from $\operatorname{lse}_f$, and absorbs the multiplicative
scalings back into additive potentials.  If $F$ and $G$ are those scaling
factors, the update is represented as
\[
    f\leftarrow f+\frac{\log F}{2k},
    \qquad
    g\leftarrow g+\frac{\log G}{2k}.
\]
The half-step absorption is the damped form used for parity with the C++
implementation; it has the same fixed point as the full log-domain updates.
The quantity called \texttt{maxdif} is the largest supported-point change,
computed from $|\log F|/k$.  We continue until \texttt{maxdif} is below
\texttt{cap\_thr} or until the relevant iteration cap is reached.
...
\]
The new generator uses the log-domain alternating updates again for each of the
...
costs into distance units.  This replaces the earlier exact equal-mass
assignment diagnostic with the independent Sinkhorn procedure used for the
paper-style push-forward error.

\subsection{Representative examples}

Figure~\ref{fig:week15-examples} contains two representative refractor
surfaces.  The angular errors are left as placeholders so that the values from
the refreshed NPZ files can be inserted directly after the generator finishes.
