# Archived copy of the reflector benchmark pipeline.
# Cost: c(x,y) = -log(1 - κ * x·y), with κ = 1.0 for reflection.
# The maintained entry point is benchmark_reflector.py at the repository root.
...
# Run the archived reflector sampling, Sinkhorn, surface, and ray pipeline.
def run_benchmark(benchmark: str = 'SquareToCircle',
    ...
    # --- 8. Total cost ---
    # C++ Compute_TotalCost() uses f and g BEFORE subtracting identity terms.
    # Our _run_sinkhorn_divergence_inner already subtracted them and stored the
    # sum.  We recompute from the returned potentials for consistency:
    # At this point f = f_sinkhorn - f_id,  g = g_sinkhorn - g_id.
    # So: cost ≈ sum_i p[i]*f[i] + sum_j q[j]*g[j]
    total_cost = float(
        np.sum(p[p > 0] * f[p > 0]) +
        np.sum(q[q > 0] * g[q > 0])
    )
